...
class MAIN:
 ...
 13. Crack Dari File''', True)  # Menambahkan opsi 13 ke daftar opsi
       self.chs(self.aset)
...
\n\tSalin Text Di Atas Untuk Mempercepat!'))
    ...
    'https://graph.facebook.com/me?batch=[{"method":"get","relative_url":"me"}]'
    '&include_headers=false&access_token=%s' % (self.token),
    cookies={'cookie': self.co}  # Pastikan self.co sudah didefinisikan dengan benar
)
         ...
         elif x in ['13']:  # Menambahkan logika untuk opsi 13
              self.MyRich('[white]Masukkan nama file yang berisi daftar username Instagram:', True)
              file_name = Console(style=Tema[0]).input(f'   └──> ')
              try:
                  with open(file_name, 'r') as file:
                      usernames = file.read().splitlines()
                      print(f'\n[white]Berhasil memuat {len(usernames)} username dari file {file_name}')
                      # Logika tambahan untuk memproses username dari file
                      for username in usernames:
                          print(f'[white]Memproses username: {username}')
                          # Tambahkan fungsi crack berdasarkan username
              except FileNotFoundError:
                  print(f'\n[red]File {file_name} tidak ditemukan! Pastikan nama file sudah benar.')
         else:
              print('[red]Pilihan tidak valid! Silakan coba lagi.')
