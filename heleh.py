...
class MAIN:
 ...
 >> User ID          : {self.ds_user} \n''',None,'[white]> [green]DETAIL USER[/] <')
  ...
  13. Crack Dari File''', True) # Tambahkan Opsi di Sini
       self.chs(self.aset)
   ...
   def chs(self, assets):
    while True:
        ...
        if x in ['01', '1']:
            self.dumps(assets, True)
        elif x in ['02', '2']:
            self.dumps(assets, False)
        elif x in ['03', '3']:
            self.Ulang()
        elif x in ['04', '4']:
            self.igrst()
            ...
            self.komentar(assets)
        elif x in ['07', '7']:
            self.Unli(assets)
        elif x in ['08', '8']:
            self.theme()
        elif x in ['09', '9']:
            exit(os.system('xdg-open https://wa.me/+6285729416714?text='))
        elif x in ['10']:
            os.system('rm -rf data/IG-login.txt')
            self.insta()
        elif x in ['11']:
            ...
            if self.fil in ['1', '01']:
                filter = 'fb'
            elif self.fil in ['2', '02']:
                filter = 'ig'
            elif self.fil in ['3', '03']:
                filter = 'tk'
            elif self.fil in ['4', '04']:
                filter = 'tw'
            elif self.fil in ['5', '05']:
                filter = 'ls'
            else:
                filter = 'ls'
            self.CariNama(self.Target, filter)
        elif x in ['12']:
            if not os.path.isfile('data/FB-login.txt'):
...
\n\tSalin Text Di Atas Untuk Mempercepat!'))
            try:
                ...
                self.req = requests.post(
                    'https://graph.facebook.com/me?batch=[{"method":"get","relative_url":"me"}]&include_headers=false&access_token=%s' % self.token,
                    cookies={'cookie': self.cokie}).json()['data']
                self.Facebook(cookies=self.cokie, token=self.token)
            except (KeyError, Exception):
                exit('\nMembutuhkan cookie baru!')
        elif x in ['13']:
            self.crack_dari_file()
        else:
            print(f'{P}[{K2}!{P}] Pilihan Tidak Tersedia')

def crack_dari_file(self):
    print(f'{P}\n[ {H}Crack Dari File{P} ]\n')
    try:
        file_path = input(f'{P}Masukkan path file target: {H}')
        with open(file_path, 'r') as file:
            targets = file.readlines()

        if len(targets) == 0:
            print(f'{K}\nFile kosong atau tidak ditemukan target.{P}')
            return

        print(f'{H}\nMemulai proses crack untuk {len(targets)} target...{P}')
        for target in targets:
            target = target.strip()
            if target:
                print(f'{H}Memproses target: {target}{P}')
                # self.Exekusi(target, self.aset, '', True)
                time.sleep(1)
        ...
        try:
            ...
            self.req = requests.post(
                'https://graph.facebook.com/me?batch=[{"method":"get","relative_url":"me"}]&include_headers=false&access_token=%s' % self.token,
                cookies={'cookie': self.cokie}).json()
        ...
        print(f'{K}\nFile tidak ditemukan. Pastikan path benar.{P}')
    except Exception as e:
        print(f'{K}\nTerjadi kesalahan: {e}{P}')
